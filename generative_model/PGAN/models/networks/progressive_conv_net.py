...
class GNet(nn.Module):
    ...
    def __init__(self,
        ...
        # encoder
        self.meanL = nn.Linear(dimEmb, dimLatent)
        self.stdvL = nn.Sequential(nn.Linear(dimEmb, dimLatent), nn.Softplus())
...
class DNet(nn.Module):
    ...
    def __init__(self,
        ...
        # set encoder
        self.encoding = nn.Linear(dimEmb, dimLatent)
    ...
    def forward(self, x, emb):
        ...
        x = x.view(-1, num_flat_features(x))
        h = self.leakyRelu(self.groupScaleZero[1](x))

        # independent critic
        out = self.decisionLayer(h)

        # conditional critic
        xc = self.encoding(emb)
        out = out + torch.sum(xc * h, 1)[:, None]
