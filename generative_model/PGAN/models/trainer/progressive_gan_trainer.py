...
class ProgressiveGANTrainer(GANTrainer):
    ...
    def __init__(self,
        ...
        GANTrainer.__init__(self, pathdb,tissue, **kwargs)
    ...
    def train(self):
        ...
        for scale in range(self.startScale, n_scales):
            ...
            dbLoader = self.getDBLoader(scale, self.tissue)
            sizeDB = len(dbLoader)
            ...
            if self.checkPointDir is not None:
                ...
                label = self.modelLabel + ("_s%d_i%d" %
                                           (scale, realIter))
                inputs_real, embs = next(iter(dbLoader))
                self.saveCheckpoint(inputs_real,
                                    embs,
                                    self.checkPointDir,
