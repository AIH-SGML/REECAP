...
class GANTrainer():
    ...
    def __init__(self,
        ...
        if (not ignoreAttribs) and \
            ...
            self.modelConfig.attribKeysOrder = self.getDataset(
                0, self.tissue, size=10).getKeyOrders()
        ...
        print("%d images detected" % int(len(self.getDataset(0, self.tissue, size=10))))
    ...
    def saveCheckpoint(self, real_data, emb, outDir, outLabel, scale, iter):
        r"""
        ...
        if self.visualisation is not None:
            ref_g = self.model.test(self.refVectorVisualization, emb)
            imgSize = max(128, ref_g.size()[2])
            img = torch.concat([ref_g[:8], real_data[:8]], 0)
            self.visualisation.saveTensor(img, (imgSize, imgSize), os.path.join(outDir, outLabel + '.jpg'))

            ref_g_smooth = self.model.test(self.refVectorVisualization, emb, True)
            img = torch.concat([ref_g_smooth[:8], real_data[:8]], 0)
            self.visualisation.saveTensor(ref_g_smooth, (imgSize, imgSize), os.path.join(outDir, outLabel + '_avg.jpg'))

    def sendToVisualization(self, refVectorReal, emb, scale, label=None):
        r"""
        ...
        ref_g = self.model.test(self.refVectorVisualization, emb, False)
    ...
    def getDBLoader(self, scale, tissue=None):
        r"""
        ...
        """
        dataset = self.getDataset(scale, tissue)
    ...
    def getDataset(self, scale, tissue=None, size=None):
        ...
        if self.path_db[-5:]=='.h5ad':
            return AnnDataset(self.path_db,
                                 transform=transform)
        ...
        for item, data in enumerate(dbLoader, 0):
            ...
            if len(data) > 2:
                ...
                allLosses = self.model.optimizeParameters(
                    inputs_real, embs, inputLabels=None, inputMasks=mask)
            else:
